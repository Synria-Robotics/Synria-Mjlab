"""Register Synria HandOver tasks."""

from mjlab.rl import MjlabOnPolicyRunner
from mjlab.tasks.registry import register_mjlab_task

from synria_mjlab.manipulation.handover.env_cfgs import handover_env_cfg
...
for name, robot_fn in _ROBOTS.items():
  register_mjlab_task(
    task_id=f"HandOver-{name}",
    env_cfg=handover_env_cfg(robot_cfg_fn=robot_fn),
    play_env_cfg=handover_env_cfg(play=True, robot_cfg_fn=robot_fn),
    rl_cfg=manip_ppo_runner_cfg(
      experiment_name=f"handover_{name.lower().replace('-', '_')}"
    ),
